...
class ProGAT(nn.Module):
    ...
    def forward(self, amino_pretrain_emb, amino_list, amino_degree_list, amino_mask):
        '''
        ...
        '''
        input_size = amino_pretrain_emb.shape[-1]
        self.emb_layer = nn.Linear(input_size, self.embedding_size)
        self.neighbor_fc = nn.Linear(input_size, self.embedding_size)
        # amino_mask: tensor(batch_size, max_seq_len, 1)
        amino_mask = amino_mask.unsqueeze(2)
        batch_size, seq_len, amino_nums = amino_list.shape
        # (512, 26) -> (512, 64)  embed amino num_amino_feature
        # amino_feature: tensor(batch_size, max_seq_len, num_amino_feature)
        amino_feature = F.leaky_relu(self.emb_layer(amino_pretrain_emb))
        # neighbor: tensor(batch, max_seq_len, max_neighbors_num, amino_nums)
        neighbor = [amino_pretrain_emb[i][amino_degree_list[i]] for i in range(batch_size)]
        neighbor = torch.stack(neighbor, dim=0)
        # (batch, 512, 25, 26) -> (batch, 512, 25, 64)
        # neighbor_feature: tensor(batch_size, max_seq_len, max_neighbors_num, num_amino_feature)
